...
def test_demo_swolfpy():
    ...
    # Treatment processes
    Treatment_processes = {}
    Treatment_processes["LF"] = {
        "input_type": ["RWC", "Bottom_Ash", "Fly_Ash", "Other_Residual"],
        "model": LF(),
    }
    Treatment_processes["WTE"] = {"input_type": ["RWC", "Other_Residual"], "model": WTE()}

    # Distance
    data = Distance.create_distance_table(
        process_names=["LF", "WTE", "SF_COl"], transport_modes=["Heavy Duty Truck"], default_dist=20
    )
    ...
    Collection_processes["SF_COl"] = {
        "input_type": [],
        "model": SF_Col(
            "SF_COl",
            Collection_scheme_SF_COL,
    ...
    # project
    demo = Project(
        project_name, common_data, Treatment_processes, distance, Collection_processes, technosphere
    )
    demo.init_project()
    demo.write_project()
    ...
    check_exchanges_with_param(
        "WTE",
        Treatment_processes["WTE"]["model"].report(),
        common_data.Index[1],
        demo.parameters.param_uncertainty_dict,
    )
    check_exchanges_with_param(
        "SF_COl",
        Treatment_processes["SF_COl"]["model"].report(),
        common_data.Index[1],
        demo.parameters.param_uncertainty_dict,
    )
