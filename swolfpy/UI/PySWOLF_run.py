...
from ..Project import *
from . import MC_ui, PySWOLF_ui, Reference_ui, adv_opt_ui
...
class MyQtApp(PySWOLF_ui.Ui_MainWindow, QtWidgets.QMainWindow):
    ...
    def init_app(self):
        ...
        self.icon.addPixmap(
            QtGui.QPixmap(":/ICONS/PySWOLF_ICONS/PySWOLF.ico"), QtGui.QIcon.Normal, QtGui.QIcon.Off
        )
    ...
    def init_load_project_tab(self):
        if not self.ini_load_project_status:
            self.Br_Project_btm.clicked.connect(
                self.select_file(self.Project_address, "Pickle (*.pickle)")
            )
            ...
            self.Show_SWM_Network_Load.clicked.connect(
                self.show_SWM_Network_func(self.Show_SWM_Network_Load_AllFlows)
            )
    ...
    def load_report_time_UpdateParam(self, time):
        ...
        self.msg_popup(
            "Parameters",
            "Parameters are updated successfully in {} seconds".format(time),
            "Information",
        )
    ...
    def Importing_processes(self):
        ...
        # CommonData
        self.helper(
            self.IT_Default_0, self.IT_UserDefine_0, self.IT_BR_0, self.IT_FName_0, "Python (*.py)"
        )
        self.helper(
            self.IT_Default_00,
            self.IT_UserDefine_00,
            self.IT_BR_00,
            self.IT_FName_00,
            "CSV (*.csv)",
        )
        ...
        # Technosphere
        self.helper(
            self.IT_Default_Tech,
            self.IT_UserDefine_Tech,
            self.IT_BR_Tech,
            self.IT_FName_Tech,
            "Python (*.py)",
        )
        self.helper(
            self.IT_Default_LCI,
            self.IT_UserDefine_LCI,
            self.IT_BR_LCI,
            self.IT_FName_LCI,
            "CSV (*.csv)",
        )
        self.helper(
            self.IT_Default_LCI_Ref,
            self.IT_UserDefine_LCI_Ref,
            self.IT_BR_LCI_Ref,
            self.IT_FName_LCI_Ref,
            "CSV (*.csv)",
        )
        self.helper_dir(
            self.IT_Default_EcoSpold2,
            self.IT_UserDefine_EcoSpold2,
            self.IT_BR_EcoSpold2,
            self.IT_FName_EcoSpold2,
        )
    ...
    def Help_ImportProcessFunc(self):
        QtGui.QDesktopServices.openUrl(
            "https://swolfpy.readthedocs.io/en/latest/Getting_started.html#import-process-models"
        )
    ...
    def load_PM_setting(self, ProcessModel):
        ...
        self.groupBox_2.setDisabled(
            self._ProcessMetaData[ProcessModel]["Process_Type"] == "Collection"
        )
    ...
    def update_PM_setting(self):
        ...
        self.msg_popup(
            "Update process model setting",
            "Settings for the {} process are updated successfully.".format(ProcessModel),
            "Information",
        )
    ...
    def Help_ColSectorFunc(self):
        QtGui.QDesktopServices.openUrl(
            "https://swolfpy.readthedocs.io/en/latest/Getting_started.html#define-collection-processes"
        )
...
""")
    ...
    def Create_collection_dict(self):
        ...
        for i in np.arange(1, self.col_index):
            x = self.Collection.findChildren(QtWidgets.QComboBox, "Col_{}".format(i))[
                0
            ].currentText()
            y = self.Collection.findChildren(QtWidgets.QLineEdit, "Col_name_{}".format(i))[0].text()
            z = (
                self.Collection.findChildren(QtWidgets.QTableView, "Sch_Col_{}".format(i))[0]
                .model()
                ._data
            )
    ...
    def set_col_scheme_table(self, Sch_Col):
        def set_process(proc):
            ...
            col_scheme_pd = pd.DataFrame(
                columns=["Contribution"], index=[str(x) for x in index], dtype=float
            )

            col_scheme_pd.fillna(0.0, inplace=True)
            col_scheme_pd_model = Table_modified_collection_schm(
                col_scheme_pd, pop_up=self.msg_popup
            )
    ...
    def init_TreatmentProcesses(self):
        ...
        self.Add_process.clicked.connect(
            self.add_process_func(self.frame_Process_treatment, self.gridLayout_101)
        )
    ...
    def Help_AddProcessFunc(self):
        QtGui.QDesktopServices.openUrl(
            "https://swolfpy.readthedocs.io/en/latest/Getting_started.html#define-treatment-processes"
        )
    ...
    def Create_Treatment_process_dict(self):
        ...
        for Index in np.arange(1, self.P_index):
            Process = self.frame_Process_treatment.findChildren(
                QtWidgets.QComboBox, "Process_" + str(Index)
            )[0]
            Process_Name = self.frame_Process_treatment.findChildren(
                QtWidgets.QLineEdit, "Process_Name_" + str(Index)
            )[0]
            Process_path = self.frame_Process_treatment.findChildren(
                QtWidgets.QLineEdit, "P_" + str(Index) + "_Process_path"
            )[0]
            Type_input = self.frame_Process_treatment.findChildren(
                QtWidgets.QCheckBox, "P_" + str(Index) + "_Type_input"
            )[0]
            Process_Label = self.frame_Process_treatment.findChildren(
                QtWidgets.QLabel, "Process_Label_" + str(Index)
            )[0]
            ...
            if Process.currentText() != "...":
                ...
                if Type_input.isChecked():
                    self._Treatment_processes[Process_Name.text()]["model"] = CLS(
                        process_name=Process_Name.text(), CommonDataObjct=self.CommonData
                    )
                else:
                    self._Treatment_processes[Process_Name.text()]["model"] = CLS(
                        process_name=Process_Name.text(),
                        input_data_path=Process_path.text(),
    ...
    def init_write_project(self):
        ...
        # Show SWM Network
        self.Show_SWM_Network.clicked.connect(
            self.show_SWM_Network_func(self.Show_SWM_Network_AllFlows)
        )
    ...
    def Help_DistanceTableFunc(self):
        QtGui.QDesktopServices.openUrl(
            "https://swolfpy.readthedocs.io/en/latest/Getting_started.html#id1"
        )
    ...
    def Create_Distance_Table(self):
        ...
        for i in range(self.spinBox.value()):
            ...
            if i > 1:
                act.addItems(
                    ["Heavy Duty Truck", "Medium Duty Truck", "Rail", "Barge", "Cargo Ship"]
                )
            else:
            ...
            columns = [x for x in self._Treatment_processes.keys()] + [
                x for x in self._Collection_processes.keys()
            ]
    ...
    def read_distance_func(self):
        ...
        for i in range(self._n_trans_mode):
            i += 1
            TableView = self.TransportWidget.findChildren(
                QtWidgets.QTableView, "Dist_Mode_{}".format(i)
            )[0]
            mode = self.TransportWidget.findChildren(QtWidgets.QComboBox, "Act_Mode_{}".format(i))[
                0
            ].currentText()
    ...
    def write_project_func(self):
        ...
        if len(self._Collection_processes) > 0:
            for i in np.arange(1, self.col_index):
                ...
                y = self.Collection.findChildren(QtWidgets.QLineEdit, "Col_name_{}".format(i))[
                    0
                ].text()
                m = self.Collection.findChildren(
                    QtWidgets.QLineEdit, "Col_input_path_{}".format(i)
                )[0].text()
                z = self.Collection.findChildren(QtWidgets.QCheckBox, "Col_def_input_{}".format(i))[
                    0
                ].isChecked()
                ...
                if x != "...":
                    ...
                    if z:
                        self._Collection_processes[y]["model"] = CLS(
                            y,
                            self._Collection_processes[y]["scheme"],
                            self._Treatment_processes,
        ...
        if (
            ...
            or self.IT_UserDefine_LCI.isChecked()
            or self.IT_UserDefine_LCI_Ref.isChecked()
            or self.IT_UserDefine_EcoSpold2.isChecked()
        ):
            LCI_path = self.IT_FName_LCI.text() if self.IT_UserDefine_LCI.isChecked() else None
            LCI_Reference_path = (
                self.IT_FName_LCI_Ref.text() if self.IT_UserDefine_LCI_Ref.isChecked() else None
            )
            Ecospold2_Path = (
                self.IT_FName_EcoSpold2.text() if self.IT_UserDefine_EcoSpold2.isChecked() else None
            )
            self.Technosphere = self._Technosphere(
                project_name=self.P_Name,
                LCI_path=LCI_path,
                LCI_Reference_path=LCI_Reference_path,
                Ecospold2_Path=Ecospold2_Path,
            )
            self.demo = Project(
                self.P_Name,
                self.CommonData,
                self._Treatment_processes,
                self.distance,
                Collection_processes=self._Collection_processes,
                Technosphere_obj=self.Technosphere,
            )
        else:
            self.demo = Project(
                self.P_Name,
    ...
    def show_SWM_Network_func(self, all_flow):
        def helper():
            self.demo.parameters.SWM_network(
                view=True, show_vals=self.param_status, all_flow=all_flow.isChecked()
            )
    ...
    def Help_CreateScenarioFunc(self):
        QtGui.QDesktopServices.openUrl(
            "https://swolfpy.readthedocs.io/en/latest/Getting_started.html#create-scenario"
        )
    ...
    def add_act_to_scenario(self):
        for i in range(len(self.process_WF._data["Name"])):
            if self.process_WF._data["Amount"].iloc[i] != 0 and not np.isnan(
                self.process_WF._data["Amount"].iloc[i]
            ):
    ...
    def create_new_scenario(self):
        ...
        for i in range(len(self.Inc_act_table._data["Process"])):
            ...
            if (
                self.Inc_act_table._data["Name"][i]
                in input_dict[self.Inc_act_table._data["Process"][i]]
            ):
                input_dict[self.Inc_act_table._data["Process"][i]][
                    self.Inc_act_table._data["Name"][i]
                ] += self.Inc_act_table._data["Amount"][i]
            else:
                input_dict[self.Inc_act_table._data["Process"][i]][
                    self.Inc_act_table._data["Name"][i]
                ] = self.Inc_act_table._data["Amount"][i]
        self.demo.create_scenario(
            input_dict=input_dict, scenario_name=self.Name_new_scenario.text()
        )
        # Notift the user that the scenario has created successfully
        self.msg_popup(
            "Create Scenario",
            "Scenario {} is created successfully".format(self.Name_new_scenario.text()),
            "Information",
        )
        self.delect_act_included()
    ...
    def LCA_tab_init(self):
        if not self.LCA_tab_init_status:
            ...
            self.DB_name_list = [
                x
                for x in databases
                if x != "biosphere3" and x != self.demo.Technosphere.user_tech_name
            ]
            ...
            self.LCA_activity.currentTextChanged.connect(
                self.load_act_unit(self.LCA_DataBase, self.LCA_FU_unit)
            )
            ...
            self.LCA_Filter_impacts.clicked.connect(
                self.Filter_Method_func(self.Filter_impact_keyword, self.LCA_method)
            )
    ...
    def LCA_CreateLCA_Func(self):
        if len(self.LCA_List_of_functional_units) > 0 and len(self.LCA_List_of_Impacts) > 0:
            calculation_setups["LCA"] = {
                "inv": self.LCA_List_of_functional_units,
    ...
    def LCA_Contribution_tab_init(self):
        ...
        if not self.LCA_Contribution_tab_init_status:
            ...
            self.LCA_Contr__Top_act.clicked.connect(
                lambda: self.LCA_Contr_Top_emis.setChecked(
                    not (self.LCA_Contr__Top_act.isChecked())
                )
            )
            self.LCA_Contr_Top_emis.clicked.connect(
                lambda: self.LCA_Contr__Top_act.setChecked(
                    not (self.LCA_Contr_Top_emis.isChecked())
                )
            )
            ...
            # Figure initialization
            self.fig_Contr_LCA = Figure(
                figsize=(4, 5), dpi=65, facecolor=(1, 1, 1), edgecolor=(0, 0, 0)
            )
    ...
    def LCA_top_act_func(self):
        ...
        top_act_data = ContributionAnalysis().annotated_top_processes(
            lca, limit=limit, limit_type="number"
        )
        ...
        if limit_type == "number":
            ...
            top_act_DF = pd.DataFrame(
                columns=["Activity", "Flow", "Flow Unit", "Contribution", "Unit"]
            )
            ...
            contribution = []
            for x in top_act_data:
                if abs(x[0]) >= float(self.LCA_Contr_CutOff.value()) * abs(
                    self.LCA_Contr_lca.score
                ):
                    Activity.append(x[2].key)
                    Flow_unit.append(x[2].as_dict()["unit"])
                    flow.append(x[1])
    ...
    def LCA_top_emission_func(self):
        ...
        lca = self.LCA_Contr_lca
        top_emission_data = ContributionAnalysis().annotated_top_emissions(
            lca, limit=limit, limit_type="number"
        )
        Emission = []
        Compartment = []
    ...
    def LCA_Contr_fig_func(self, DF, legend=None):
        ...
        # set lables
        self.ax_Contr_LCA.set_title(
            "Contribution to " + str(self.LCA_Contr_lca.method), fontsize=18
        )
        ...
        if legend:
            self.ax_Contr_LCA.legend(legend, fontsize=18, bbox_to_anchor=(1, 0, 0.2, 1), loc=2)
        else:
            self.ax_Contr_LCA.legend(fontsize=18, bbox_to_anchor=(1, 0, 0.2, 1), loc=2)
    ...
    def MC_tab_init(self):
        if not self.MC_tab_init_status:
            ...
            self.MC_FU_act.currentTextChanged.connect(
                self.load_act_unit(self.MC_FU_DB, self.MC_FU_unit)
            )
            ...
            self.MC_Filter_method.clicked.connect(
                self.Filter_Method_func(self.MC_Filter_keyword, self.MC_method)
            )
            ...
            CheckBoxes = ["CommonData", "Parameters"] + [
                x for x in self.demo.Treatment_processes.keys()
            ]
    ...
    def Help_UncertaintyDistFunc(self):
        QtGui.QDesktopServices.openUrl(
            "https://swolfpy.readthedocs.io/en/latest/Getting_started.html#uncertainty-distribution"
        )
    ...
    def MC_load_uncertain_func(self, process_name):
        if process_name in self.demo.Treatment_processes.keys():
            self.uncertain_data = deepcopy(
                self.demo.Treatment_processes[process_name]["model"].InputData.Data
            )
    ...
    def MC_uncertain_filter_func(self):
        if self.MC_uncertain_filter.isChecked():
            MC_Uncertain_model = Table_from_pandas_editable(
                self.uncertain_data[:][self.uncertain_data["uncertainty_type"] > 1],
                pop_up=self.msg_popup,
            )
        else:
            MC_Uncertain_model = Table_from_pandas_editable(
                self.uncertain_data, pop_up=self.msg_popup
            )
    ...
    def MC_uncertain_update_func(self):
        if self.MC_Model.currentText() in self.demo.Treatment_processes.keys():
            self.demo.Treatment_processes[self.MC_Model.currentText()][
                "model"
            ].InputData.Update_input(self.MC_Uncertain_table.model()._data)
    ...
    def MC_uncertain_clear_func(self):
        ...
        self.MC_Uncertain_table.model()._data[
    ...
    def show_res_func(self):
        ...
        self.fig_plot_corr = Figure(
            figsize=(4, 10), dpi=65, facecolor=(1, 1, 1), edgecolor=(0, 0, 0)
        )
    ...
    def mc_plot_corr_func(self):
        ...
        # set lables
        self.ax_corr.set_xlabel(
            "Correlation with {}".format(self.MC_Widget.Corr_Impact.currentText()), fontsize=18
        )
    ...
    def mc_plot_func(self):
        ...
        # ploting the DataFrame
        self.ax_plot_mc = self.MC_results.plot(
            kind="scatter" if self.MC_Widget.scatter.isChecked() else "hexbin",
    ...
    def mc_plot_dist_func(self):
        ...
        # ploting the DataFrame
        if kind == "hist":
            self.ax_plot_dist_mc = self.MC_results[
                self.MC_results.columns[self.MC_Widget.param.currentIndex()]
            ].plot(kind=kind, ax=self.ax_plot_dist_mc, bins=max(30, len(self.MC_results) / 100))
        else:
            self.ax_plot_dist_mc = self.MC_results[
                self.MC_results.columns[self.MC_Widget.param.currentIndex()]
            ].plot(kind=kind, ax=self.ax_plot_dist_mc)
    ...
    def Opt_tab_init(self):
        if not self.Opt_tab_init_status:
            ...
            self.Opt_FU_act.currentTextChanged.connect(
                self.load_act_unit(self.Opt_FU_DB, self.Opt_FU_unit)
            )
            ...
            self.Opt_Filter_method.clicked.connect(
                self.Filter_Method_func(self.Opt_Filter_keyword, self.Opt_method)
            )
            ...
            self.Opt_Const2_process.currentIndexChanged.connect(
                self.load_db_func(self.Opt_Const2_flow)
            )
    ...
    def Opt_add_const1(self):
        if self.Opt_Const1_process.currentText() != "...":
            self.Const_DF.loc[self.Const_DF_index] = [
                self.Opt_Const1_process.currentText(),
                self.Opt_Const1_Inequality.currentText(),
                float(self.Opt_Const1_val.text()),
            ]
            self.Const_DF_index += 1
            self.Opt_Const_table_update()
            self.constraints[self.Opt_Const1_process.currentText()] = {
                "limit": float(self.Opt_Const1_val.text()),
                "KeyType": "Process",
    ...
    def Opt_add_const2(self):
        if self.Opt_Const2_process.currentText() != "...":
            self.Const_DF.loc[self.Const_DF_index] = [
                (self.Opt_Const2_process.currentText(), self.Opt_Const2_flow.currentText()),
                self.Opt_Const2_Inequality.currentText(),
                self.Opt_Const2_val.text(),
            ]
            self.Const_DF_index += 1
            self.Opt_Const_table_update()
            self.constraints[
                ...
                "limit": float(self.Opt_Const2_val.text()),
    ...
    def Opt_add_const3(self):
        if self.Opt_Const3_flow.currentText() != "...":
            self.Const_DF.loc[self.Const_DF_index] = [
                self.Opt_Const3_flow.currentText(),
                self.Opt_Const3_Inequality.currentText(),
                self.Opt_Const3_val.text(),
            ]
            self.Const_DF_index += 1
            self.Opt_Const_table_update()
            self.constraints[self.bio_dict[self.Opt_Const3_flow.currentText()]] = {
                "limit": float(self.Opt_Const3_val.text()),
                "KeyType": "Emission",
...
constraints = {self.constraints}""")
        ...
        optimizer_thread = Worker_Optimize(
            parent=self.Opt_optimize,
            opt=self.opt,
    ...
    def Opt_draw_sankey_func(self, params=None):
        ...
        if params:
            ...
            self.opt.plot_sankey(
                optimized_flow=True, show=False, fileName=os.getcwd() + "\\Optimized_sankey.html"
            )
                ...
                if len(rows) == 1 and len(columns) == 1:
                    for i, line in enumerate(reader):
                        for j, cell in enumerate(line):
                            ...
                            if (rows[0] + i) < model.rowCount() and (
                                columns[0] + j
                            ) < model.columnCount():
    ...
    def save(self):
        """
        Https://stackoverflow.com/questions/15416334/qfiledialog-how-to-set-default-
        filename-in-save-as-dialog.
        """
        cwd = os.getcwd()
    ...
    def Filter_Method_func(self, keyWord_lineEdit, Method_ComboBox):
        def Filter_Method_func_helper():
            list_methods = [
                str(x) for x in methods if (keyWord_lineEdit.text().lower() in str(x).lower())
            ]
            list_methods.sort()
    ...
    def on_downloadRequested(self, download):
        """
        Https://stackoverflow.com/questions/55963931/how-to-download-csv-file-with-
        qwebengineview-and-qurl.
        """
        old_path = download.url().path()
        path, _ = QtWidgets.QFileDialog.getSaveFileName(self, "Save File", old_path, "*.png")
        if path:
    ...
    def Ref_filter_func(self):
        ...
        references = Table_from_pandas(
            self.RefDF[[kwrd.lower() in str(x).lower() for x in self.RefDF["Reference"]]]
        )
        self.References.TableRef.setModel(references)
        self.References.TableRef.installEventFilter(self)
        self.References.TableRef.setWordWrap(True)
